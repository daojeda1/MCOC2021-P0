# -*- coding: utf-8 -*-
"""
Created on Fri Sep  3 16:59:34 2021
...
def graficar(Ns, dt_ens, dt_solv, dtype):
    ...
    plt.figure(1)
    plt.subplot(2,1,1)
    plt.title("Rendimiento"+" INV "+str(dtype))
...
############################################################################   
    ...
    # plt.legend(loc="lower left")
    plt.axis([1,max(Ns),0.0001,100])#ACA IMPORTANTE DESCOMENTARjfshdgjrhgoernñoi
###############################################
...
##############################################################################
    ...
    # plt.axis([1,max(Ns),0.0001,2])############
    plt.yticks(valor2,palabras2)
    plt.axis([1,max(Ns),0.0001,0.1]) #Lalalalalalala
    ...
    # plt.grid(True)
    plt.savefig("Rendimiento Inv" + " "+ str(dtype) +".jpg")  
