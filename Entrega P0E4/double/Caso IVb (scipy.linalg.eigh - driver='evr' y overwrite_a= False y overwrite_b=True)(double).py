# -*- coding: utf-8 -*-
"""
Created on Fri Aug 20 16:44:22 2021
...
dt_inv_temporal = []
mems = []
libreria = "(Caso IVb - scipy.linalg.solve "
dtype = "float32)"
caso = "IVb"
...
for i in range(10):
    for N in Ns:
        ...
        B = b(N,dtype = float32)
        t4=perf_counter()
        w, v = linalg.eigh(A,driver='evr', overwrite_a=False, overwrite_b=True)
        t5=perf_counter()
        
        dt_ensamblaje_A = t2-t1
        dt_inversion_A = t3-t2
        dt_solve = t5-t2
        bytes_total = A.nbytes + B.nbytes + w.nbytes + v.nbytes
        dts.append(dt_solve)
