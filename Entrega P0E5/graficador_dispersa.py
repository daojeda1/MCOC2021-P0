# -*- coding: utf-8 -*-
"""
Created on Thu Aug 26 21:31:43 2021
...
###############################################
    ...
    for i in range(len(cortes)):    
        plt.plot([cortes[i],Ns[-1]],[dt_ens[0][0]**exp[i],max(dt_ens[-1])],linestyle = "--",label=casos[i]) #IMPORTANTE###################################################
        plt.legend(loc="lower left")
    # plt.axhline(y=dt_solv[-1][-1],xmin=0.0,xmax=Ns[-1],linestyle="--",color='black', label = "Constante") #CONSTANTE RAM
    # plt.legend(loc="lower left")
    plt.axis([0,max(Ns),0.0001,60])
###############################################
    ...
    # plt.legend(loc="lower left")
    plt.axis([0,max(Ns),0.0001,60])
    # plt.axhline(y=dt_solv[-1][-1],xmin=0.0,xmax=Ns[-1],linestyle="--",color='black') #CONSTANTE RAM
